...
from Meshtastic_Relay import (
    ...
    parse_minutes,
    _to_int_safe,
      # si tu helper usa esta constante
)
...
# Helper: formatear lista detallada de nodos
def format_nodes_list(nodes: List[Tuple[str, str, int, Optional[int]]]) -> Tuple[List[str], Dict[str, str]]:
    """
    Devuelve:
      - lines: ["1. alias (!id) — visto hace Xm — hops: Y", ...]
      - mapping: {"1": "!id", "!id": "!id", ...} para resolver número o ID
    """
    lines: List[str] = []
    mapping: Dict[str, str] = {}
    for i, (nid, alias, mins, hops) in enumerate(nodes, start=1):
        label = alias or nid
        line = f"{i}. {label} ({nid}) — visto hace {mins} min"
        if hops is not None:
            line += f" — hops: {hops}"
        lines.append(line)
        mapping[str(i)] = nid
...
def sync_nodes_and_save(n_max: int = 20) -> List[str]:
    ...
    fn = _relay_has("get_visible_nodes_with_hops")
    if fn:
        try:
...
def request_telemetry(node_id: str, timeout: int = TELEMETRY_TIMEOUT) -> str:
    ...
    # Fallback CLI: siempre pasamos primero --dest
    flags_try = [
        ["--dest", node_id, "--request-telemetry"],
        ["--dest", node_id, "--requestPosition"],
...
async def ayuda(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        "📚 *Ayuda rápida*\n\n"
        "/ver_nodos [N] – muestra los últimos N nodos vistos (predet. 20), numerados con “visto hace X min” y “hops: Y”.\n"
        "/traceroute <número|!id> – muestra la ruta completa hacia el nodo (usa índice o ID).\n"
        "/telemetria <número|!id> – solicita datos de telemetría al nodo (índice o ID).\n"
        "/enviar <número|!id|broadcast> <texto…> – envía un texto al nodo indicado o a todos.\n"
        "/escuchar [canal|all] – escucha mensajes de texto entrantes (TEXT_MESSAGE_APP) en el broker:\n"
        "    • sin args → canal predeterminado\n"
        "    • `/escuchar 5` → solo canal 5\n"
        "    • `/escuchar all` → todos los canales\n"
        "/parar_escucha – detiene la escucha activa del broker.\n"
        "/vecinos <número|!id> – muestra vecinos directos (1 salto) del nodo.\n"
        "/estado – comprueba estado del broker y del bot.\n"
...
# ---- Nodos / Vecinos
def load_nodes_prefer_relay_con_hops(n_max: int = 20) -> List[Tuple[str, str, int, Optional[int]]]:
    """
+    1) Llama a sync_nodes_and_save(n_max) para forzar fetch (relay o CLI) y volcar NODES_FILE.
+    2) Vuelve a parsear NODES_FILE con _parse_nodes_table().
+    3) Devuelve [(id, alias, minutos, hops|None), …] ordenado asc por minutos.
+    """
    # 1) Fuerza fetch → deja tabla en disco y filas formateadas en memoria (pero no nos importan)
    sync_nodes_and_save(n_max)  # :contentReference[oaicite:0]{index=0}

    # 2) Parseamos el fichero crudo de nodos
    rows = _parse_nodes_table(NODES_FILE)
    out: List[Tuple[str, str, int, Optional[int]]] = []
    for r in rows:
        nid   = r.get("id", "")
        alias = r.get("alias", "") or nid
        mins  = parse_minutes(r.get("last_seen_text", "") or "")
        # Intentamos extraer hops (tabla reporta "hops_text")
        hops_val = _to_int_safe(r.get("hops_text"))
        out.append((nid, alias, mins, hops_val))
...
async def ver_nodos_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not raw:
        # Si por algún motivo no hay nada (relay+CLI fallan), fallback puro:
        rows_fallback = sync_nodes_and_save(n_max=n)
        lines = [f"{i+1}. {ln}" for i, ln in enumerate(rows_fallback)]
        mapping = {
            str(i+1): re.search(r"(![0-9a-fA-F]{8})", ln).group(1)
            for i, ln in enumerate(rows_fallback) if re.search(r"![0-9a-fA-F]{8}", ln)
        }
    else:
        lines, mapping = format_nodes_list(raw)
    # guardamos el mapping para usos posteriores
    context.user_data["nodes_map"] = mapping
    # enviamos al usuario
    header = "📡 Últimos nodos visibles (usa número o !id):\n"
    for chunk in chunk_text(header + "\n".join(lines)):
...
def _resolve_node_id(text: str, context) -> str:
     ...
     """
     Dado un texto que el usuario ha escrito (número o !id), devuelve el !id real.
     """
     mapping = context.user_data.get("nodes_map", {})
...
async def traceroute_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        return
    node_id = _resolve_node_id(context.args[0].strip(), context)
...
async def enviar_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(args) >= 2:
        ...
        node = None if dest.lower() == "broadcast" else _resolve_node_id(dest, context)
...
async def escuchar_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if arg and arg.lower() in ("all", "*"):
        ...
        try:
            listen_chan = int(arg) if arg else BROKER_CHANNEL
        except ValueError:
            listen_chan = BROKER_CHANNEL
        canal_msg = f"canal {listen_chan}"

    # 2) (Re)crear BrokerClient si no existe o cambió el canal
     # (Re)crear BrokerClient si no existe o si el canal deseado cambió
    if BROKER is None or BROKER.channel != listen_chan:
        # Nuevo callback que filtra solo TEXT_MESSAGE_APP
        async def _forward_broker(chat_id: int, raw: str):
            try:
                data = json.loads(raw)
            except:
                return
            # Filtramos solo paquetes de texto
            ...
            texto  = decoded.get("payload", "")
            origen = pkt.get("fromId", "(desconocido)")
            canal  = pkt.get("channel", "(desconocido)")

            # Y enviamos solo este mensaje al chat
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"📩 Mensaje de {origen} en canal {canal}:\n{texto}"
            )

        BROKER = BrokerClient(BROKER_HOST, BROKER_PORT, listen_chan, _forward_broker)

    # 3) Registrar este chat y arrancar escucha
    BROKER.add_chat(update.effective_chat.id)
    await BROKER.start()

    # 4) Confirmación al usuario
    await update.effective_message.reply_text(
        f"👂 Escuchando broker {BROKER_HOST}:{BROKER_PORT} en {canal_msg} para este chat."
    )
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "ver_nodos":
       # Delegamos al mismo handler que atiende /ver_nodos
...
async def on_forcereply_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("await_send_dest"):
        ...
        context.user_data.pop("await_send_dest", None)
         # convertimos número a !id si hace falta
        dest = _resolve_node_id(text, context)
        context.user_data["send_dest_menu"] = dest
